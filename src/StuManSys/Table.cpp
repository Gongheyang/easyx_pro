...
Table::Table(const Manager& manager, int x, int y, 
...
void Table::show() const
{
	...
	for (int j = 40; j < 370; j += 30)
	{
		if (i < manager.size())	// 在不超出索引范围的情况下才读取数据
		{
			...
			_itow_s(manager.getStudent(i)._id, buffer, 10);
			drawtext(buffer, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

			rect = { 130, j, 130 + 120, j + 30 };
			drawtext(manager.getStudent(i)._name, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

			rect = { 250, j, 250 + 120, j + 30 };
			drawtext(manager.getStudent(i)._class, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

			rect = { 370, j, 370 + 120, j + 30 };
			_itow_s(manager.getStudent(i)._score, buffer, 10);
			drawtext(buffer, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
...
void Table::showTable(int index)
{
	...
	_itow_s(manager.getStudent(index)._id, buffer, 10);
	drawtext(buffer, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

	rect = { 130, 40, 130 + 120, 70 };
	drawtext(manager.getStudent(index)._name, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

	rect = { 250, 40, 250 + 120, 70 };
	drawtext(manager.getStudent(index)._class, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

	rect = { 370, 40, 370 + 120, 70 };
	_itow_s(manager.getStudent(index)._score, buffer, 10);
	drawtext(buffer, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
}
